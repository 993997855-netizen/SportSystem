...
async function decorateSession(session, studentId) {
  const [bookedRows, trialCount, enrollment, waiting, leaveRows, classResult, attendanceRows] = await Promise.all([
    db.collection("enrollments").where({ sessionId: session._id, status: "booked" }).limit(100).get(),
  ...
  const bookedIds = new Set(bookedRows.data.map((item) => item.studentId)); const expected = bookedRows.data.length;
  const attendanceStats = { expected, present: 0, leave: 0, injured: 0, absent: 0, unmarked: expected };
  attendanceRows.data.filter((record) => bookedIds.has(record.studentId)).forEach((record) => { const key = record.status === "sick" ? "injured" : record.status; if (key in attendanceStats && key !== "expected" && key !== "unmarked") { attendanceStats[key] += 1; attendanceStats.unmarked = Math.max(0, attendanceStats.unmarked - 1); } });
  ...
  const clubClass = classResult.data || {}; const totalCount = expected + trialCount.total;
  return { ...publicDoc(session), classType: clubClass.classType || "REGULAR", classTypeLabel: clubClass.classType === "ELITE" ? "精英队" : "普通班", enrolledCount: expected, trialCount: trialCount.total, totalCount, remaining: Math.max(0, Number(session.capacity || 0) - totalCount), isFull: totalCount >= Number(session.capacity || 0), attendanceStats, myStatus: leaveStatus || (enrollment.data.length ? "booked" : position ? "waiting_history" : "none"), leaveRequestId: leave ? leave._id : "", waitlistPosition: position };
}
...
async function getSession(user, id, studentId) {
  ...
  const [enrollments, waiting, attendance] = await Promise.all([db.collection("enrollments").where({ sessionId: id, status: "booked" }).limit(100).get(), db.collection("waitlist").where({ sessionId: id, status: "waiting" }).orderBy("createdAt", "asc").limit(100).get(), db.collection("attendance").where({ sessionId: id }).limit(100).get()]);
  const students = await fetchByIds("students", [...enrollments.data, ...waiting.data].map((item) => item.studentId));
  return { ...decorated, enrollments: enrollments.data.map((item) => ({ ...publicDoc(item), attendanceStatus: (attendance.data.find((record) => record.studentId === item.studentId) || {}).status || "unmarked", student: publicDoc(students.find((s) => s._id === item.studentId)) })), waiting: waiting.data.map((item, index) => ({ ...publicDoc(item), position: index + 1, student: publicDoc(students.find((s) => s._id === item.studentId)) })) };
}
...
async function requestLeave(user, input) {
  requireRole(user, ["admin", "parent"]); const studentId = validId(input.studentId || (user.studentIds || [])[0]); await assertStudentAccess(user, studentId);
  const session = (await db.collection("sessions").doc(validId(input.sessionId)).get()).data; const enrollment = await db.collection("enrollments").where({ sessionId: input.sessionId, studentId, status: "booked" }).limit(1).get(); if (!session || !enrollment.data.length) throw new Error("该学员尚未报名本课程"); const [duplicate, approved] = await Promise.all([db.collection("leaveRequests").where({ sessionId: input.sessionId, studentId, status: "pending" }).limit(1).get(), db.collection("leaveRequests").where({ sessionId: input.sessionId, studentId, status: "approved" }).limit(1).get()]); if (duplicate.data.length) throw new Error("请假申请已提交"); if (approved.data.length) throw new Error("该课程请假已经批准");
...
async function getAttendanceSheet(user, input) {
  ...
  const [enrollments, records, approvedLeaves] = await Promise.all([db.collection("enrollments").where({ sessionId: input.sessionId, status: "booked" }).limit(100).get(), db.collection("attendance").where({ sessionId: input.sessionId }).limit(100).get(), db.collection("leaveRequests").where({ sessionId: input.sessionId, status: "approved" }).limit(100).get()]); const students = await fetchByIds("students", enrollments.data.map((item) => item.studentId));
