...
const COLLECTIONS = ["users", "students", "classes", "classMembers", "eliteSelections", "sessions", "enrollments", "waitlist", "leaveRequests", "attendance", "lessonLedger", "feedback", "renewals", "invites", "auditLogs", "leads", "leadFollowUps", "trialBookings", "assessmentTemplates", "assessmentRounds", "playerAssessments", "playerGrowthEvents", "playerMatchRecords", "tournaments", "leagues", "leagueSeasons", "leagueRounds", "teams", "teamMembers", "seasonTeams", "externalPlayers", "matches", "matchSquads"];
...
exports.main = async (event) => {
  try {
    await ensureCollections(); await classService.ensureMigration(); await growthService.ensureDefaults(); await leagueService.ensureDefaults(); const user = await ensureUser(cloud.getWXContext().OPENID); const input = event.data || {}; let data;
