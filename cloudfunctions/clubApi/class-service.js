...
function createClassService({ db, fetchAll, fetchByIds, publicDoc, nowText, requireRole, audit, getCoachReference, coachScope }) {
  function eliteRecommendForbidden() {
    const error = new Error("只能推荐自己负责班级的ACTIVE正式学员");
    error.code = "COACH_ELITE_RECOMMEND_FORBIDDEN";
    return error;
  }
  let migrationReady;
  ...
  async function assertClassAccess(user, classId) {
    ...
    if (user.role === "coach") return coachScope.assertClassAccess(user, classId);
  ...
  async function memberView(member, student, classes, user) {
    ...
    const visibleClasses = user.role === "coach" ? ownClasses.filter((item) => coachScope.coachPrincipalId(user) && [item.headCoachUserId || item.coachUserId, ...(item.assistantCoachIds || [])].includes(coachScope.coachPrincipalId(user))) : ownClasses;
    return { ...publicDoc(member), student: { ...base, initial: (student.name || "学")[0], birthYear: String(student.birthDate || "").slice(0, 4), classNames: visibleClasses.map((item) => item.name).join("、"), regularClassNames: visibleClasses.filter((item) => item.classType === "REGULAR").map((item) => item.name).join("、"), trainingLevel: visibleClasses.some((item) => item.classType === "ELITE") ? "精英队" : "普通班" }, statusLabel: MEMBER_STATUS[member.status] || member.status };
  }
  ...
  async function call(action, input, user) {
    ...
    if (action === "getClassMeta") { const allActive = await fetchAll("classes", { status: "ACTIVE" }); const ownIds = user.role === "coach" ? new Set(await coachScope.assignedClassIds(user)) : null; const rows = user.role === "coach" ? allActive.filter((item) => ownIds.has(item._id) || item.classType === "ELITE") : allActive; const decorated = []; for (const row of rows) { const item = await decorateClass(row); decorated.push(user.role === "coach" ? coachScope.coachClassView(item) : item); } return { classTypes: CLASS_TYPES, memberStatuses: MEMBER_STATUS, selectionStatuses: SELECTION_STATUS, exitReasons: user.role === "admin" ? EXIT_REASONS : [], regularClasses: decorated.filter((item) => item.classType === "REGULAR" && (!ownIds || ownIds.has(item.id))), eliteClasses: decorated.filter((item) => item.classType === "ELITE") }; }
    if (action === "getClassDetail") { await assertClassAccess(user, input.id); const clubClass = (await db.collection("classes").doc(input.id).get()).data; if (!clubClass) throw new Error("班级不存在"); if (user.role === "parent") return parentClassSummary(clubClass); const members = await fetchAll("classMembers", { classId: input.id }); const filtered = members.filter((item) => input.includeInactive && user.role === "admin" || item.status === "ACTIVE"); const students = await fetchByIds("students", filtered.map((item) => item.studentId)); const classes = await fetchAll("classes"); const memberRows = []; for (const member of filtered) { const student = students.find((item) => item._id === member.studentId); if (student) memberRows.push(await memberView(member, student, classes, user)); } const pendingSelectionCount = user.role === "admin" ? (await db.collection("eliteSelections").where({ targetEliteClassId: input.id, status: "PENDING" }).count()).total : 0; const detail = { ...(await decorateClass(clubClass)), members: memberRows, pendingSelectionCount }; return user.role === "coach" ? coachScope.coachClassView(detail) : detail; }
    ...
    if (action === "recommendElite") { requireRole(user, ["admin", "coach"]); const target = (await db.collection("classes").doc(input.targetEliteClassId).get()).data; if (!target || target.classType !== "ELITE") throw new Error("请选择目标精英队"); if (user.role === "coach") { try { await coachScope.assertClassAccess(user, input.fromClassId); } catch (error) { throw eliteRecommendForbidden(); } const member = (await db.collection("classMembers").where({ classId: input.fromClassId, studentId: input.studentId, status: "ACTIVE" }).limit(1).get()).data[0]; if (!member) throw eliteRecommendForbidden(); } const reason = String(input.recommendationReason || "").trim(); if (!reason) throw new Error("请填写推荐理由"); const duplicate = (await db.collection("eliteSelections").where({ studentId: input.studentId, targetEliteClassId: target._id, status: "PENDING" }).limit(1).get()).data[0]; if (duplicate) throw new Error("该学员已有待审核推荐"); const createdAt = nowText(); const data = { studentId: input.studentId, fromClassId: input.fromClassId || "", targetEliteClassId: target._id, recommendationSource: user.role === "coach" ? "COACH_RECOMMENDATION" : "ADMIN_RECOMMENDATION", recommendedBy: user._id, recommenderName: user.name, recommendationReason: reason, trainingPerformance: String(input.trainingPerformance || "").trim(), technicalPerformance: String(input.technicalPerformance || "").trim(), matchPerformance: String(input.matchPerformance || "").trim(), status: "PENDING", createdAt, updatedAt: createdAt }; const added = await db.collection("eliteSelections").add({ data }); await audit(user, "recommendElite", "eliteSelection", added._id, { operator: user._id, studentId: data.studentId, fromClassId: data.fromClassId, toClassId: data.targetEliteClassId, reason }); return { id: added._id, status: data.status }; }
    if (action === "listEliteSelections") { requireRole(user, ["admin", "coach"]); let rows = await fetchAll("eliteSelections"); if (user.role === "coach") rows = rows.filter((item) => item.recommendedBy === user._id); const students = await fetchByIds("students", rows.map((item) => item.studentId)), classes = await fetchByIds("classes", rows.flatMap((item) => [item.fromClassId, item.targetEliteClassId])); return rows.map((item) => { const fromClass = classes.find((clubClass) => clubClass._id === item.fromClassId), targetClass = classes.find((clubClass) => clubClass._id === item.targetEliteClassId); return { ...publicDoc(item), student: user.role === "coach" ? coachScope.coachStudentView(students.find((student) => student._id === item.studentId)) : publicDoc(students.find((student) => student._id === item.studentId)), fromClass: user.role === "coach" ? coachScope.coachClassView(fromClass) : publicDoc(fromClass), targetClass: user.role === "coach" ? coachScope.coachClassView(targetClass) : publicDoc(targetClass), statusLabel: SELECTION_STATUS[item.status] || item.status }; }).sort((a, b) => String(b.createdAt).localeCompare(String(a.createdAt))); }
