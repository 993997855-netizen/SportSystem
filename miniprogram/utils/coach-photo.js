async function chooseCoachPhoto() {
  ...
  if (getApp().globalData.dataMode !== "cloud") return localPath;
  const extension = (localPath.match(/\.[a-zA-Z0-9]+$/) || [".jpg"])[0].toLowerCase();
  const cloudPath = `coach-photos/internal/${new Date().toISOString().slice(0, 10)}/${Date.now()}-${Math.floor(Math.random() * 100000)}${extension}`;
  const uploaded = await wx.cloud.uploadFile({ cloudPath, filePath: localPath });
  return uploaded.fileID;
}

module.exports = { chooseCoachPhoto };
