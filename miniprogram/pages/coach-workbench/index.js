...
Page({
  data: { loading: true, hasLoaded: false, error: "", date: "", range: "today", timetable: { items: [], stats: {} }, allItems: [], groups: [] },
  onLoad() { wx.setNavigationBarTitle({ title: "今日教学" }); this.setData({ date: today() }); }, onShow() { this.load(); }, onPullDownRefresh() { api.clearCache("getUnifiedTimetable"); this.load(); },
  async load() { if (!this.data.date) return; const loadId = (this._loadId || 0) + 1; this._loadId = loadId; if (!this.data.hasLoaded) this.setData({ loading: true, error: "" }); try { const timetable = await api.call("getUnifiedTimetable", { date: this.data.date }); if (loadId !== this._loadId) return; this.setData({ timetable, allItems: timetable.items || [], loading: false, hasLoaded: true, error: "" }, () => this.applyRange()); } catch (error) { if (loadId === this._loadId) this.setData({ loading: false, error: "教学安排加载失败，请稍后重试" }); } finally { wx.stopPullDownRefresh(); } },
