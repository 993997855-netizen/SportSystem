...
Page({
  ...
  async load(refresh = false) {
    ...
    try {
      ...
      this.setData({ role: context.user.role, students, studentIndex, studentId, sessions: sessions.map((item) => ({ ...item, shortDate: String(item.date || "").slice(5), statusLabel: item.myStatus === "booked" ? "正式成员" : item.myStatus === "leave_pending" ? "请假待审批" : item.myStatus === "leave_approved" ? "已请假 · 0课时" : item.myStatus === "leave_rejected" ? "请假被拒绝" : item.classType === "ELITE" && context.user.role === "parent" ? "选拔制" : item.isFull ? "本班已满" : "可报名" })), loading: false });
