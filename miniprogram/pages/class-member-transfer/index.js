...
Page({
  ...
  async onLoad(options) { const meta = await api.call("getClassMeta"); const classes = [...meta.regularClasses, ...meta.eliteClasses].filter((item) => item.id !== options.classId); this.setData({ memberId: options.memberId || "", sourceClassId: options.classId || "", classes, loading: false }); },
