...
Page({
  ...
  async load() {
    ...
    try { const [context, raw] = await Promise.all([api.call("getContext"), api.call("getSession", { id: this.data.id, studentId: this.data.studentId })]); const session = { ...raw, enrollments: (raw.enrollments || []).map((item) => ({ ...item, student: { ...item.student, initial: item.student && item.student.name ? item.student.name[0] : "学" } })) }; this.setData({ role: context.user.role, session, progress: session.standardCapacity ? Math.min(100, Math.round(session.memberCount / session.standardCapacity * 100)) : 0, loading: false }); }
