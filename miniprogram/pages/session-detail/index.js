...
Page({
  ...
  async primary() {
    ...
    if (session.myStatus === "waiting") return wx.showToast({ title: `候补第${session.waitlistPosition}位`, icon: "none" });
    if (session.myStatus === "leave_pending") return wx.showToast({ title: "请假正在审核", icon: "none" });
    if (session.myStatus === "booked") return this.leave();
    this.setData({ acting: true }); try { const result = await api.call("enrollSession", { sessionId: session.id, studentId: this.data.studentId }); wx.showModal({ title: result.status === "waiting" ? "已加入候补" : "报名成功", content: result.message, showCancel: false, success: () => this.load() }); } finally { this.setData({ acting: false }); }
  },
  leave() {
