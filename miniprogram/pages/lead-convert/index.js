...
Page({
  ...
  async onLoad(options) {
    const [lead, allClasses, meta] = await Promise.all([api.call("getLead", { id: options.leadId }), api.call("listClasses"), api.call("getCrmMeta")]);
    const classes = allClasses.filter((item) => item.classType === "REGULAR"), packages = Object.keys(meta.packages).map((id) => ({ id, ...meta.packages[id] })), recommended = lead.trials && lead.trials[0] && lead.trials[0].recommendedClassId, classIds = recommended && classes.some((item) => item.id === recommended) ? [recommended] : [], coachIndex = Math.max(0, meta.coaches.findIndex((item) => item.id === lead.ownerCoachId));
  ...
  async perform(confirmDuplicate) {
    ...
    const result = await api.call("convertLead", { id: this.data.leadId, avatarUrl: this.data.avatarUrl, classIds: this.data.classIds, packageId: pack.id, initialLessons: pack.lessons, amount: pack.amount, registrationDate: this.data.registrationDate, ownerCoachId: coach.id, ownerCoachName: coach.name, confirmDuplicate });
    ...
    wx.showToast({ title: "已转正式学员" }); setTimeout(() => wx.redirectTo({ url: `/pages/student-detail/index?id=${result.id}` }), 450);
  },
